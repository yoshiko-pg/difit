...
| `--port <port>`         | 4966         | 선호 포트; 사용 중인 경우 +1로 대체                                                 |
| `--host <host>`         | 127.0.0.1    | 서버를 바인딩할 호스트 주소 (외부 액세스는 0.0.0.0 사용)                            |
| `--no-open`             | false        | 브라우저를 자동으로 열지 않음                                                       |
| `--mode <mode>`         | split        | 표시 모드: `unified` 또는 `split`                                                   |
| `--preview-mode <mode>` | diff-preview | Markdown/Notebook 파일의 기본 미리보기 모드: `diff`, `diff-preview`, `full-preview` |
